...
resource "null_resource" "provision" {
  ...
  connection {
    type     = "winrm"
    host     = azurerm_windows_virtual_machine.example.private_ip_address
    user     = "beheerder"
  ...
  provisioner "remote-exec" {
    inline = [
      "powershell.exe -Command \"& {Start-Sleep -Seconds 30}\"", # Wait for the machine to be ready
